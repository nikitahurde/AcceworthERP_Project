...
<style>
  ...
  <div class="content-wrapper">
        ...
        <section class="content">
          ...
          <a href="{{ url('finance/transaction/production/production') }}" class="btn btn-primary" style="margin-right: 10px;"><i class="fa fa-plus"></i>&nbsp;&nbsp;Add Production(BOM)</a>
   ...
   $(document).ready(function(){
    ...
    var t = $("#example").DataTable({
       ...
       ajax:{
        ...
        url : "{{ url('/finance/transaction/production/view-production') }}"
               ...
               success:function(data){
                  ...
                  if (data1.response == 'error') {
                      ...
                      if(data1.data==''){
                         ...
                         var srNo=1;
                         var tableid = objrow[0].production_head_id ;
                       $.each(objrow, function (row, objrow) {

                          if(objrow.qty_recvd){

                            var qty_recvd = objrow.qty_recvd;
                          }else{
                            var qty_recvd = '--';
                          }
                          ...
                          if(objrow.qty_issue){
                            ...
                            var qty_issue = objrow.qty_issue;
                          }else{
                            var qty_issue = '--';
                          }
                          ...
                          if(objrow.aq_issue){
                            ...
                            var aq_issue = '--';
                               ...
                               $('#childData_'+tableid).append('<tr><td class="text-center">'+srNo+'</td><td><p>'+objrow.item_name+' </p><p style="line-height: 2px;">( '+objrow.item_code+')</p></td><td class="text-right">'+qty_recvd+'</td><td class="text-right">'+aq_recvd+'</td><td class="text-right">'+qty_issue+'</td><td class="text-right">'+aq_issue+'</td><td><a class="btn btn-warning btn-xs" title="edit" disabled><i class="fa fa-pencil" title="Edit"></i></a> | <button class="btn btn-danger btn-xs" data-toggle="modal" onclick="return dealerDelete('+objrow.id+');" disabled><i class="fa fa-trash" title="Delete"></i></button></td></tr>');
                              srNo++;
                             });
