...
<style>
  ...
  <div class="content-wrapper">
        ...
        <section class="content">
          ...
          <a href="{{ url('/finance/transaction/sales-order-transaction') }}" class="btn btn-primary" style="margin-right: 10px;"><i class="fa fa-plus"></i>&nbsp;&nbsp;Add Sales Order Trans</a>
   ...
   $(document).ready(function(){
    ...
    var t = $("#example").DataTable({
       ...
       ajax:{
        ...
        url : "{{ url('/finance/transaction/view-sales-order-transaction') }}"
       ...
       columns: [
         ...
         {  
            render: function (data, type, full, meta){
                    ...
                    if(full['inward_trans']==1 || full['sap_bill']==1 || full['fleet_trans']==1 || full['acc_mast']==1)
                    {
                      var disableBtn = 'disabled';
                      var deletebtn ='<a href="edit-account-type/'+Base64.encode(full['id'])+'/'+Base64.encode(disableBtn)+'" class="btn btn-warning btn-xs" title="edit" disabled><i class="fa fa-pencil" title="Edit"></i></a> | <button class="btn btn-danger btn-xs" data-toggle="modal" disabled><i class="fa fa-trash" title="Delete"></i></button>';
                     }else{
                      var enableBtn = 'enable';
                      var deletebtn ='<a href="edit-account-type/'+Base64.encode(full['id'])+'/'+Base64.encode(enableBtn)+'" class="btn btn-warning btn-xs" title="edit" disabled><i class="fa fa-pencil" title="Edit"></i></a> | <button class="btn btn-danger btn-xs" data-toggle="modal" onclick="return getId('+full['id']+');" disabled><i class="fa fa-trash" title="Delete"></i></button>';
                     }
...
function showchildtable(vrno,tblid){
            var vrno,tblid;
             ...
             $.ajax({
              ...
              url:"{{ url('view-sale-order-chield-row-data') }}",
               ...
               success:function(data){
                  ...
                  if (data1.response == 'error') {
                      ...
                      if(data1.data==''){
                         ...
                         var srNo=1;
                         var tableid = objrow[0].sales_order_head_id;
                       $.each(objrow, function (i, objrow) {
                               $('#childData_'+tableid).append('<tr><td class="text-right">'+srNo+'</td><td>'+objrow.item_code+'</td><td>'+objrow.item_name+'</td><td class="text-right">'+objrow.quantity+'</td><td class="text-right">'+objrow.Aquantity+'</td><td class="text-right">'+objrow.rate+'</td><td class="text-right">'+objrow.basic_amt+'</td></tr>');
                              srNo++;
                             });
