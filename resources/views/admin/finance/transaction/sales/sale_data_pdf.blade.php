          ...
          <?php  $sum = 0; $bal=0; $sr_no=1; foreach($dataheadB as $key) {
           ...
           ?>
            ...
            <tr style="padding-top: 20%;">
              ...
              <td class="bodyTextS" style="width:19%;">{{$key->ITEM_NAME}}  <?php if($key->PARTICULAR){echo '[ '.$key->PARTICULAR.' ]';}?></td>
              ...
              <td class="bodyTextS textRightS" style="width:14%;">{{ $aQuantity }}</td>
              <td style="width:6%;">{{ $key->AUM }}</td>
...
<?php if($pdfName == 'SALES BILL'){ ?>
    <table class="table table_highlight" style="width: 100%;" style="margin-bottom: 0px;">
      ...
      <tr>
        ...
        <td>:</td>
        <td>
          Interest e18% P.A. Will Be Charged If Not Paid Within Due Date
        </td>
      </tr>
      <tr>
        <td>3</td>
        <td>:</td>
        <td>
         Any Dispatch, Subject To <?php echo $compDetail[0]->CITY; ?> Juridiction Only.
        </td>
      </tr>
    </table>
<?php }else{ ?>
