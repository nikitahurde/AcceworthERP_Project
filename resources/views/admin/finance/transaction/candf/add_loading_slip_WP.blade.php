              ...
              <table class="table tdthtablebordr" border="1" cellspacing="0" id="bodytbledata">
                ...
                <tr>
                  ...
                  <td class='tdthtablebordr'>
                    ...
                    <datalist id='batchNoList1'>
                      ...
                      <?php foreach ($orderNo_list as $key) { ?>
                          ...
                          <option value='<?php echo $key->BATCH_NO.'~'.$key->SLNO.'~'.$key->CFINWARDID;?>' data-xyz ="<?php echo $key->BATCH_NO.'~'.$key->SLNO.'~'.$key->CFINWARDID; ?>" ><?php echo $key->BATCH_NO.'-'.$key->ITEM_NAME.'-'.$key->LOADBALQTY.'-'.$key->WAGON_NO.'-'.$key->INVOICE_NO.'-'.$key->RAKE_NO.'-'.$key->LENGTH.'-'.$key->WIDTH.'-'.$key->HEIGHT;?></option>
                  ...
                  <td class='tdthtablebordr' >
                    ...
                    <input type="hidden" id="available_Addqty1">
                  </td>
                  <td class='tdthtablebordr' >
                  ...
                  <td class='tdthtablebordr' >
                    ...
                    <input type="hidden" id="available_qty1">
                  </td>
                  <td class='tdthtablebordr' >
...
<!--  -------- MODAL SHOW WHEN ENTER QTY GREATER THAN AVAILABLE QTY --------- -->

  <div id="greaterQtyShModel1" class="modal fade" tabindex="-1">
    ...
    <div class="modal-dialog modal-sm" style="margin-top: 13%;">
      ...
      <div class="modal-content" style="border-radius: 5px;">
        ...
        <div class="modal-body table-responsive">
            <p style="line-height: 1;">Quantity Should Not Be Greater Than Available Quantity</p>
        </div>

        <div class="modal-footer" style="text-align: center;" id="greatQtyFooter1">
              ...
              console.log(data1.data[0]);
                  ...
                  $.each(data1.data, function(k, getData){
                    ...
                    $("#batchNoList1").append($('<option>',{
                      ...
                      'text':getData.BATCH_NO+'-'+getData.ITEM_NAME+'-'+getData.LOADBALQTY+'-'+getData.WAGON_NO+'-'+getData.INVOICE_NO+'-'+getData.RAKE_NO+'-'+getData.LENGTH+'-'+getData.WIDTH+'-'+getData.HEIGHT
  ...
  $(".addmore").on('click',function(){
    ...
    count=$('table tr').length;
     ...
     var data = "<tr><td class='tdthtablebordr'><input type='hidden' id='tempItemSave"+i+"' value=''><input type='checkbox' class='case' id='firstrow"+i+"' onclick='checkcheckbox("+i+");'/></td>"+
        ...
        "<td class='tdthtablebordr' ><input class='inputboxclr numerRight aQtyVal' type='text' value='' name='addQuantity[]' id='addquantity"+i+"' autocomplete='off' oninput='calcQty("+i+",\""+'AQTY'+"\")'/><input type='hidden' id='available_Addqty"+i+"'></td>"+
        ...
        "<td class='tdthtablebordr'><input class='inputboxclr numerRight qtyVal' type='text' value='' name='quantity[]' id='quantity"+i+"' autocomplete='off' oninput='calcQty("+i+",\""+'QTY'+"\")'/><input type='hidden' name='cFactor[]' id='cFactor"+i+"' value='0'><input type='hidden' id='available_qty"+i+"'></td>"+
        ...
        "<td class='tdthtablebordr'><input type='text' name='cp_name[]' id='cp_name"+i+"' readonly class='inputboxclr readonlyField'  value='' /><div id='greaterQtyShModel"+i+"' class='modal fade' tabindex='-1'><div class='modal-dialog modal-sm' style='margin-top: 13%;'><div class='modal-content' style='border-radius: 5px;'><div class='modal-header'><div class='row'><div class='col-md-12'><h5 class='modal-title modltitletext' id=''  style='color: red;font-weight: 800;'>Alert</h5></div></div></div><div class='modal-body table-responsive'><p style='line-height: 1;'>Quantity Should Not Be Greater Than Available Quantity</p></div><div class='modal-footer' style='text-align: center;' id='greatQtyFooter"+i+"'><button type='button' class='btn btn-primary' data-dismiss='modal'>Ok</button></div></div></div></div></td>"+
...
<script>
  ...
  function calcQty(slno,fieldType){
    ...
    var avl_qty     = parseFloat($('#available_qty'+slno).val());
    var avl_Addqty  = parseFloat($('#available_Addqty'+slno).val());
    ...
    if(fieldType=='QTY'){
      ...
      if(quantity){
        ...
        if(quantity >avl_qty){
          $('#greaterQtyShModel'+slno).modal('show');
          $('#quantity'+slno).val(avl_qty)
          //var aqtyRound = Math.round(Avlaqty);
          $('#addquantity'+slno).val(avl_Addqty);
        }
            ...
            $('#errorItem').html("<p style='color:red'>"+ data1.message +"</p>");
              ...
              if(data1.data_inwardDt_list == ''){
                ...
                if(batchNo && itemCode && orderNo && fieldType=='ORDER'){
                  ...
                  $('#available_qty'+slNo).val(loadBalQty.toFixed(3));
                  $('#um_code'+slNo).val(data1.data_inwardDt_list[0].UM);
                  $('#addquantity'+slNo).val(aQtyRound);
                  $('#available_Addqty'+slNo).val(aQtyRound);
