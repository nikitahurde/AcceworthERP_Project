            ...
            <form id="salesordertrans">
              ...
              <div class="table-responsive">
                ...
                <table class="table tdthtablebordr" border="1" cellspacing="0" id="tbledata">
                  ...
                  <input type ="hidden" name="account_name" value="<?php if($accCount == 1){echo $getacc[0]->ACC_NAME;}else{} ?>" id="getAcc_Name">
                  ...
                  <input type ="hidden" name="plant_name" id="getPlantName" value="<?php if($plcount == 1){echo $getplant[0]->PLANT_NAME;}else{}?>">
                  ...
                  <input type ="hidden" name="contractNum" id="getcontractNo">
                  <input type ="hidden" name="quoationNum" id="getquotionNo">
  ...
  display: none;
      ...
      <br>
        ...
        <p class="text-center">
          ...
          <button class="btn btn-success" type="button" id="submitNDwn" onclick="submitAllData(1);" disabled><i class="fa fa-floppy-o" aria-hidden="true"></i>&nbsp;&nbsp; Save & Download</button>
...
<script type="text/javascript">
  ...
  function getContraQuo(){
    ...
    $.ajax({
            ...
            data: {account_code: account_code,transDate:transDate},
            ...
            success:function(data){
              ...
              var data1 = JSON.parse(data);
                ...
                if (data1.response == 'error') {
                  ...
                  console.log('data1.contradata',data1.contradata);
                  if(data1.contradata== ''){
                 ...
                 // $('#appndplantbtn').append('<button type="button"  data-toggle="modal" data-target="#plant_detail" onclick="getplantdata()" class="btn btn-xs btn-info gly-radius" data-original-title="" title="" style="padding: 0px 5px 0px 5px;"> <i class="fa fa-info" aria-hidden="true" style="font-size: 13px;"></i></button>');
                   ...
                   if(data1.acc_adddata == ''){

                   }else{

                      $("#shipTAdd").empty();
                      $.each(data1.acc_adddata, function(k, gettax){
  ...
  function getItmByQtnNContra(){
      ...
      $.ajax({
          ...
          data: {account_code: account_code,contractNo:contractNo,quotationNo:quotationNo,seriesByTrn:seriesByTrn},
                ...
                if (data1.response == 'error') {
                  ...
                  //  $('#tax_code').prop('readonly',false);
                    $('#party_ref_date').val('');
                      ...
                      if(data1.data[0].DUEDATE){

                        var endDate = data1.data[0].DUEDATE;
                        var dateform = endDate.split('-');
                        var properEndDate = dateform[2]+'-'+dateform[1]+'-'+dateform[0];

                        var prefDate = data1.data[0].PREFDATE;
                        var prefdateform = prefDate.split('-');
                        var propPRefDate = prefdateform[2]+'-'+prefdateform[1]+'-'+prefdateform[0];

                        var diffInMs   = new Date(endDate) - new Date(startDate);
                        var diffInDays = diffInMs / (1000 * 60 * 60 * 24);

                        $('#due_days').val(diffInDays).css('border-color','#d2d6de');
                        $('#dueDays_err').html('');
                      ...
                      $('#party_ref_date').val(propPRefDate).prop('disabled',true);
                      $('#getpartyrfDate').val(propPRefDate);
                      $('#tax_code').val(data1.data[0].TAX_CODE);
...
function getitmByTax(){
    ...
    var Quotn_compare_no = $('#quotationNo').val();
    ...
    $.ajax({
            ...
            data: {taxCode: taxCode,Quotn_compare_no:Quotn_compare_no,account_code:account_code,contractNo:contractNo},
                ...
                if (data1.response == 'error') {
                  ...
                  $('#errorItem').html("<p style='color:red'>"+ data1.message +"</p>");
                   ...
                   if(data1.data ==''){
                    ...
                    if(Quotn_compare_no || contractNo){
...
<script type="text/javascript">
            ...
            if (data1.response == 'error') {
              ...
              //console.log('data1.data[0]',data1.data[0]);
                if(data1.data == ''){
                      ...
                      if(contractNo){
                        var tableHead= "<div class='box-row'><div class='box10 texIndbox'>#</div><div class='box10 rateIndbox' style='width: 6%;'>Vr. No</div><div class='box10 rateIndbox'>Item Name</div><div class='box10 rateIndbox'>Contract Qty </div><div class='box10 rateIndbox'>Order Qty </div><div class='box10 rateIndbox'>Cancle Qty </div><div class='box10 rateIndbox'>Contract Bal. Qty </div></div>";
                      }else if(quotationNo){
                        var tableHead= "<div class='box-row'><div class='box10 texIndbox'>#</div><div class='box10 rateIndbox' style='width: 6%;'>Vr. No</div><div class='box10 rateIndbox'>Item Name</div><div class='box10 rateIndbox'>Quo. Qty </div><div class='box10 rateIndbox'>Order Qty </div><div class='box10 rateIndbox'>Cancle Qty </div><div class='box10 rateIndbox'>Quo. Bal. Qty </div><div class='box10 rateIndbox'>Level </div></div>";
                      }
                      ...
                      $.each(data1.data, function(k, getData) {
                        ...
                        if(getData.QUOCOMPNO){
                            var qcnovrno = getData.QUOCOMPNO;
                        }else if(getData.PCVRNO){

                          var startyear = getData.FY_CODE;
                          var getyear = startyear.split("-");
                          var qcnovrno = getyear[0]+'&nbsp;'+getData.SERIES_CODE+'&nbsp;'+getData.PCVRNO;
                        }

                        if(getData.PCNTRHID){
                          var pContra_HID=getData.PCNTRHID;
                        }else{
                          var pContra_HID='';
                        }

                        if(getData.PCNTRBID){
                          var pContra_BID=getData.PCNTRBID;
                        }else{
                          var pContra_BID='';
                        }

                        if(getData.PURQNHID){
                          var pQuo_HID=getData.PURQNHID;
                        }else{
                          var pQuo_HID='';
                        }

                        if(getData.PURQNBID){
                          var pQuo_BID=getData.PURQNBID;
                        }else{
                          var pQuo_BID='';
                        }

                        if(getData.QUOCOMPNO){
                          var pQuoComp_HID=getData.QUOCOMPNO;
                        }else{
                          var pQuoComp_HID='';
                        }

                        if(getData.PURQCSBID){
                          var pQuoComp_BID=getData.PURQCSBID;
                        }else{
                          var pQuoComp_BID='';
                        }
                        ...
                        if(quotationNo){
                         ...
                         var level_td = "<div class='box10 rateIndbox'><input type='text' id='qlevel_"+itemval+"_"+incemntval+"' name='' class='form-control rightcontent' value="+getData.LEVEL+" readonly></div>";
                        }else{
                          var level_td ='';
                        }
        
                        var tableBody = "<div class='box-row' id='hidebalNull_"+itemval+"_"+incemntval+"'><div class='box10 texIndbox'><input type='radio' id='sr_"+itemval+"_"+incemntval+"' name='itemname' value='"+itemval+"_"+incemntval+"'><input type='hidden' id='contHeadId_"+itemval+"_"+incemntval+"' value="+pContra_HID+"><input type='hidden' id='contbodyId_"+itemval+"_"+incemntval+"' value="+pContra_BID+"><input type='hidden' id='contVrno_"+itemval+"_"+incemntval+"' value="+getData.VRNO+"><input type='hidden' id='contslno_"+itemval+"_"+incemntval+"' value="+getData.SLNO+"><input type='hidden' id='QuoHead_"+itemval+"_"+incemntval+"' value="+pQuo_HID+"><input type='hidden' id='QuoBody_"+itemval+"_"+incemntval+"' value="+pQuo_BID+"><input type='hidden' id='qCompnum_"+itemval+"_"+incemntval+"' value="+pQuoComp_HID+"><input type='hidden' id='qCompbody_"+itemval+"_"+incemntval+"' value="+pQuoComp_BID+"></div><div class='box10 texIndbox_vr'><input type='text' id='vrno_"+itemval+"_"+incemntval+"' name='head_tax_ind11[]' class='form-control inputtaxInd ' value="+qcnovrno+" readonly></div><div class='box10 texIndbox_itm tooltips'><input type='text' id='itemcode_"+itemval+"_"+incemntval+"' name='poitemname[]' class='form-control inputtaxInd' value='"+getData.ITEM_CODE+"("+getData.ITEM_NAME+")"+"' readonly><input type='hidden' id='taxCodeI_"+itemval+"_"+incemntval+"' value="+getData.TAX_CODE+"><div class='tooltiptextitem tooltiphide' id='itemNameTooltip_"+itemval+"_"+incemntval+"'></div><input type='hidden' id='itemname_"+itemval+"_"+incemntval+"' name='poitemcode[]' class='form-control inputtaxInd' value='"+getData.ITEM_NAME+"' readonly><input type='hidden' id='discriptin_"+itemval+"_"+incemntval+"' name='' class='form-control inputtaxInd' value='"+getData.PARTICULAR+"' readonly></div><div class='box10 rateIndbox'><input type='text' id='qtyOreder_"+itemval+"_"+incemntval+"' name='qtyOrder' class='form-control rightcontent' value="+getData.QTYRECD+" readonly><input type='hidden' value="+getData.RATE+" id='rateqNo_"+itemval+"_"+incemntval+"'><input type='hidden' value="+getData.AQTYRECD+" id='addqtyqc_"+itemval+"_"+incemntval+"'></div><div class='box10 rateIndbox'><input type='text' id='qtySupply_"+itemval+"_"+incemntval+"' name='qtySupply[]' class='form-control rightcontent' value="+qtyissued+" readonly></div><div class='box10 rateIndbox'><input type='text' id='qtyCancle_"+itemval+"_"+incemntval+"' name='qtyCancle[]' class='form-control rightcontent' value="+getData.QTYCANCEL+" readonly></div><div class='box10 rateIndbox'><input type='text' id='balence_qty_"+itemval+"_"+incemntval+"' name='balQty[]' class='form-control rightcontent' value="+getData.QTYRECD+" readonly><input type='hidden' class='form-control' id='remainBalQty_"+itemval+"_"+incemntval+"' value='' readonly><input type='hidden' class='form-control' id='tolindex"+incemntval+"' value="+getData.TOL_INDEX+" readonly><input type='hidden' class='form-control' id='tolrate"+incemntval+"' value="+getData.TOL_RATE+" readonly><input type='hidden' class='form-control' id='tolpervalue"+incemntval+"' value="+getData.tol_value+" readonly></div>"+level_td+"</div>";
  ...
  function selectitem(rowid,itmebyid){
   ...
   if(checkitmIsAval == ''){
    ...
    var contHeadId  = $('#contHeadId_'+res1+'_'+res2).val();
    var contBodyId  = $('#contbodyId_'+res1+'_'+res2).val();
    var contVrNo    = $('#contVrno_'+res1+'_'+res2).val();
    var contSlNo    = $('#contslno_'+res1+'_'+res2).val();
    var QuoHeadId   = $('#QuoHead_'+res1+'_'+res2).val();
    var QuoBodyId   = $('#QuoBody_'+res1+'_'+res2).val();
    var QuoCompNum  = $('#qCompnum_'+res1+'_'+res2).val();
    var QuoCompBody = $('#qCompbody_'+res1+'_'+res2).val();
    var deiscriptn  = $('#discriptin_'+res1+'_'+res2).val();
    var tax_codeI   = $('#taxCodeI_'+res1+'_'+res2).val();

    if(deiscriptn == null || deiscriptn == 'null' || deiscriptn ==''){
        var remarkPc = '';
      }else{
        var remarkPc = deiscriptn;
      }
    ...
    $('#slContraHead'+rowid).val(contHeadId);
    $('#slContraBody'+rowid).val(contBodyId);
    $('#slQuoHead'+rowid).val(QuoHeadId);
    $('#slQuoBody'+rowid).val(QuoBodyId);
    $('#slQuoCompHead'+rowid).val(QuoCompNum);
    $('#slQuoCompBody'+rowid).val(QuoCompBody);
    $('#taxByItem'+rowid).val(tax_codeI);
    $('#Item_CodeId'+rowid).val(getitemCd);
    $('#itmGetCode'+rowid).val(getitemCd);
    $('#selectItem'+rowid).val(getitemCd);
    $('#remark_data'+rowid).val(remarkPc);
              ...
              var data1 = JSON.parse(data);
                ...
                if (data1.response == 'error') {
                  ...
                  if(data1.data == ''){
                      ...
                      $('#getPfctName').val(data1.data[0].PFCT_CODE);
...
<script type="text/javascript">
  ...
  $(".addmore").on('click',function(){
      ...
      var contractNo = getconranum[2];
      var seiresbytrans = getconranum[1];
      ...
      var taxCode = $('#tax_code').val();
      var quoNo = $('#quotationNo').val();
      var contract_No = $('#contractNo').val();
      ...
      $.ajax({
            ...
            data: {taxCode: taxCode,Quotn_compare_no:Quotn_compare_no,account_code:account_code,contractNo:contractNo,seiresbytrans:seiresbytrans},
  ...
  function submitAllData(valD){
      ...
      if(foundItm == ''){
          ...
          $.ajax({
              ...
              url: "{{ url('/Transaction/Purchase/Save-Purchase-Order-Trans') }}",
