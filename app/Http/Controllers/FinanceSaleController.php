...
/* ---- start sale enquiry transaction ---- */
	...
	public function SalesEnquiryTrans(Request $request){
		...
		$userdata['series_list'] = DB::table('MASTER_CONFIG')->where('COMP_CODE',$getcompcode)->where('TRAN_CODE','S0')->get();

		$userdata['item_list']   = DB::table('MASTER_ITEM')->where('COMP_CODE',$getcompcode)->orWhere('COMP_CODE','')->orWhere('COMP_CODE',NULL)->get();

		$acc_list                =  DB::table('MASTER_ACC')->get();
		$tax_code_list           =  DB::table('MASTER_TAX')->get();

		$userdata['plant_list']  = DB::table('MASTER_PLANT')->where('COMP_CODE',$getcompcode)->get()->toArray();
	...
	public function SaveSalesEnquiry(Request $request){
		...
		$vrno_Exist = DB::table('SENQ_HEAD')->where('SERIES_CODE',$series_code)->where('COMP_CODE',$compCode)->where('FY_CODE',$fisYear)->where('VRNO',$vrNum)->get()->toArray();
...
/*------ start sales quotation -----*/
	...
	public function AddsalesQuotation(Request $request)
    {
		...
		$transCode   = 'S1';
		$userdata['item_list']   = DB::table('MASTER_ITEM')->where('COMP_CODE',$compCode)->orWhere('COMP_CODE','')->orWhere('COMP_CODE',NULL)->get();
		...
		$userdata['plant_list'] = $getCommonData['masterPlant'];
    ...
    public function SaveSalesQuotation(Request $request){
		...
		$vrno_Exist = DB::table('SQTN_HEAD')->where('SERIES_CODE',$series_code)->where('COMP_CODE',$compCode)->where('FY_CODE',$fisYear)->where('VRNO',$vrNum)->get()->toArray();
...
/*---- start sale contract transaction ----*/
    ...
    public function SaveSaleContractTrans(Request $request){
		...
		$vrno_Exist = DB::table('SCNTR_HEAD')->where('SERIES_CODE',$series_code)->where('COMP_CODE',$getcom_code)->where('FY_CODE',$fisYear)->where('VRNO',$vrNum)->get()->toArray();
...
/*---- sale order transaction ----*/
    ...
    public function SaveSalesOrder(Request $request){
		...
		$vrno_Exist = DB::table('SORDER_HEAD')->where('SERIES_CODE',$series_code)->where('COMP_CODE',$getcom_code)->where('FY_CODE',$fisYear)->where('VRNO',$vrNum)->get()->toArray();
...
/*DIRECT SALE TRANSACTION*/
     ...
     public function SaveDirectSalesTrans(Request $request){
		...
		$vrno_Exist = DB::table('SBILL_HEAD')->where('SERIES_CODE',$series_code)->where('COMP_CODE',$getcom_code)->where('FY_CODE',$fisYear)->where('VRNO',$vrNum)->get()->toArray();
...
/*---- sale bill transaction ----*/
	...
	public function SaveSaleTrans(Request $request){
		...
		if ($request->ajax()) {
			...
			$vrno_Exist = DB::table('SBILL_HEAD')->where('SERIES_CODE',$series_Code)->where('COMP_CODE',$compcode)->where('FY_CODE',$MaccYear)->where('VRNO',$vrNum)->get()->toArray();
		...
		try {
			...
			$checkvrnoExist = DB::table('MASTER_VRSEQ')->where('TRAN_CODE',$transcode)->where('SERIES_CODE',$series_Code)->where('COMP_CODE',$compcode)->where('FY_CODE',$MaccYear)->get()->toArray();
			...
			if(empty($checkvrnoExist)){
				...
				DB::table('MASTER_VRSEQ')->where('TRAN_CODE',$transcode)->where('SERIES_CODE',$series_Code)->where('COMP_CODE',$compcode)->where('FY_CODE',$MaccYear)->update($datavrn);
			}
...
/* ------- start post good challan -------- */
    ...
    public function SavePostGoddIssue(Request $request){
		...
		$vrno_Exist = DB::table('SCHALLAN_HEAD')->where('SERIES_CODE',$series_code)->where('COMP_CODE',$getcom_code)->where('FY_CODE',$fisYear)->where('VRNO',$vrNum)->get()->toArray();
...
public function GeneratePdfForSale($userId,$getcom_code,$headId,$headtable,$bodytable,$taxtable,$columnheadid,$pdfName,$vrPName,$tCode){
		...
		$compcode     =  $getcom_code;

		$response_array = array();

		DB::table('SIMULATION_TEMP')->where('CREATED_BY',$userId)->where('TCFLAG','RT')->orWhere('TCFLAG','R_IT')->delete();
		//DB::enableQueryLog();
		$dataheadB = DB::SELECT("SELECT t1.*,MASTER_ACC.ACC_NAME,MASTER_ACC.ACC_CODE,'$headtable' as tablNme,t2.*,t3.SERIES_CODE,t3.SERIES_NAME,t4.GST_NO,t4.CITY_NAME as plant_city FROM $headtable t1 LEFT JOIN MASTER_ACC ON MASTER_ACC.ACC_CODE = t1.ACC_CODE LEFT JOIN $bodytable t2 ON t2.$columnheadid = t1.$columnheadid LEFT JOIN MASTER_CONFIG t3 ON t3.SERIES_CODE=t1.SERIES_CODE AND t3.TRAN_CODE=t1.TRAN_CODE AND t3.COMP_CODE=t1.COMP_CODE LEFT JOIN MASTER_PLANT t4 ON t4.COMP_CODE=t1.COMP_CODE AND t4.PLANT_CODE=t1.PLANT_CODE WHERE t2.$columnheadid='$headId'");
		...
		$dataConfig = DB::table('MASTER_CONFIG')->where('TRAN_CODE',$tCode)->where('SERIES_CODE',$seriesCode)->where('COMP_CODE',$compcode)->get()->toArray();
...
/* ------------ download pdf for sale view -------------------*/
	...
	public function pdfDownloadForViewSales(Request $request){
		...
		if ($request->ajax()) {
			...
			if($compDetail){
				$compAccount = $compDetail[0]->ACC_CODE;
				if($compAccount){

					$bankAccDetail = DB::SELECT("SELECT * FROM MASTER_ACC WHERE ACC_CODE='$compAccount'");
				}else{
					$bankAccDetail='';
				}
			}else{
			...
			$this->ConvertAmountIntoWordForView($grandAmt,$tCode,$seriesCode,$dataheadB,$dataTax,$taxDetail,$pdfName,$dataConfig,$dataAccDetail,$consinerDetail,$compDetail,$vrPName,$seirsHeadLine,$bankAccDetail);
	...
	public function ConvertAmountIntoWordForView($grandAmt,$tCode,$seriesCode,$dataheadB,$dataTax,$taxDetail,$pdfName,$dataConfig,$dataAccDetail,$consinerDetail,$compDetail,$vrPName,$seirsHeadLine,$bankAccDetail){
    	...
    	$pdf = PDF::loadView('admin.finance.transaction.sales.sales_voucher_data_report',compact('dataheadB','dataTax','taxDetail','pdfName','dataConfig','dataAccDetail','consinerDetail','compDetail','vrPName','seirsHeadLine','numwords','bankAccDetail'));
