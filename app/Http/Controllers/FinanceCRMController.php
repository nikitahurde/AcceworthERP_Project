...
class FinanceCRMController extends Controller
{
...
*/
    ...
    public function ViewCrmEnquiryTransaction (Request $request){
        ...
        if($request->ajax()) {
           ...
           $data =  DB::select("SELECT t1.VRNO,t1.TRAN_CODE,t1.VRDATE,t1.SERIES_CODE,t1.SENQHID,t1.QTNCOMP_STATUS,t2.SERIES_NAME FROM SENQ_HEAD t1 LEFT JOIN MASTER_CONFIG t2 ON t2.SERIES_CODE = t1.SERIES_CODE WHERE t1.ACC_CODE ='$acc_code'
   						UNION
     			 SELECT t3.VRNO,t3.TRAN_CODE,t3.VRDATE,t3.SERIES_CODE,t3.CRMENQHID,t3.QTNCOMP_STATUS,t4.SERIES_NAME FROM CRM_ENQ_HEAD t3 LEFT JOIN MASTER_CONFIG t4 ON t4.SERIES_CODE = t3.SERIES_CODE WHERE CRM_ENQ_HEAD.ACC_CODE ='$acc_code'");

            }else if($userType=='superAdmin' || $userType=='user' || $userType=='CRM'){
          ...
          // 	DB::enableQueryLog();
			   ...
			   $data =  DB::select("SELECT t1.VRNO,t1.TRAN_CODE,t1.VRDATE,t1.SERIES_CODE,t1.SENQHID,t1.QTNCOMP_STATUS,t2.SERIES_NAME FROM SENQ_HEAD t1 LEFT JOIN MASTER_CONFIG t2 ON t2.SERIES_CODE = t1.SERIES_CODE WHERE t1.ACC_CODE ='$acc_code'
   						UNION
     			 SELECT t3.VRNO,t3.TRAN_CODE,t3.VRDATE,t3.SERIES_CODE,t3.CRMENQHID,t3.QTNCOMP_STATUS,t4.SERIES_NAME FROM CRM_ENQ_HEAD t3 LEFT JOIN MASTER_CONFIG t4 ON t4.SERIES_CODE = t3.SERIES_CODE WHERE t3.ACC_CODE ='$acc_code'");
        ...
        return view('admin.finance.transaction.crm.view_crm_enquiry_transaction');
...
/*------ start sales quotation -----*/
	...
	public function ViewCrmQuotation(Request $request){
	     ...
	     if($request->ajax()) {
	        ...
	        $userType = $request->session()->get('usertype');
           	...
           	if($userType=='superAdmin' || $userType=='user' || $userType=='CRM'){

              
	           $data =DB::select("SELECT SQTN_HEAD.*,SQTN_BODY.SQTNHID as salehid,SQTN_BODY.SQTNBID,SQTN_BODY.SCNTRHID,SQTN_BODY.SCNTRBID,MASTER_CONFIG.SERIES_NAME,MASTER_PLANT.PLANT_NAME,MASTER_ACC.ACC_NAME,group_concat(concat(SQTN_BODY.SCNTRHID))AS SCTRSTATUSHD,group_concat(concat(SQTN_BODY.SCNTRBID))AS SCTRSTATUSBD FROM SQTN_HEAD LEFT JOIN MASTER_CONFIG ON MASTER_CONFIG.SERIES_CODE=SQTN_HEAD.SERIES_CODE LEFT JOIN MASTER_PLANT ON MASTER_PLANT.PLANT_CODE=SQTN_HEAD.PLANT_CODE LEFT JOIN MASTER_ACC ON MASTER_ACC.ACC_CODE=SQTN_HEAD.ACC_CODE LEFT JOIN SQTN_BODY ON SQTN_BODY.SQTNHID = SQTN_HEAD.SQTNHID WHERE SQTN_HEAD.ACC_CODE ='$acc_code' GROUP BY SQTN_HEAD.SQTNHID");
	       ...
	       return view('admin.finance.transaction.crm.view_crm_quotation_trans');
    ...
    public function ViewCrmOrderTransaction(Request $request){
     	...
     	if($request->ajax()) {
	        ...
	        $userType = $request->session()->get('usertype');
	          ...
	          $data =DB::select("SELECT SORDER_HEAD.*,SORDER_BODY.SORDERHID as salehid,SORDER_BODY.SORDERBID,SORDER_BODY.SCHALLANHID,SORDER_BODY.SCHALLANBID,MASTER_CONFIG.SERIES_NAME,MASTER_PLANT.PLANT_NAME,MASTER_ACC.ACC_NAME,group_concat(concat(SORDER_BODY.SCHALLANHID))AS SCHLANSTATUSHD,group_concat(concat(SORDER_BODY.SCHALLANBID))AS SCHLANSTATUSBD FROM SORDER_HEAD LEFT JOIN MASTER_CONFIG ON MASTER_CONFIG.SERIES_CODE=SORDER_HEAD.SERIES_CODE LEFT JOIN MASTER_PLANT ON MASTER_PLANT.PLANT_CODE=SORDER_HEAD.PLANT_CODE LEFT JOIN MASTER_ACC ON MASTER_ACC.ACC_CODE=SORDER_HEAD.ACC_CODE LEFT JOIN SORDER_BODY ON SORDER_BODY.SORDERHID = SORDER_HEAD.SORDERHID WHERE SORDER_HEAD.ACC_CODE = '$acc_code'   GROUP BY SORDER_HEAD.SORDERHID");
	        }
	        else{
    		...
    		return DataTables()->of($data)->addIndexColumn()->addColumn('action', function($data){
       		...
       		return view('admin.finance.transaction.crm.view_crm_order');
	...
	public function CrmGrnReport(Request $request){


        $title = "Crm Grn Report";
       ...
       // $transpoter_list = DB::table('MASTER_ACC')->get();
        ...
        $acc_list        = DB::table('MASTER_ACC')->get();
            ...
            return view('admin.finance.transaction.crm.crm_grn_report',$userdata+compact('title','bank_list','item_list','acc_list','yearstart','yearend'));
    ...
    public function getDataFromQueryFormCrmGrn(Request $request){
