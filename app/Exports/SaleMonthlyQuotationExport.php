...
class SaleMonthlyQuotationExport implements FromCollection, WithHeadings,ShouldAutoSize,WithCustomStartCell,WithEvents
{
    /**
...
//DB::enableQueryLog();
     ...
     /*$data1 = DB::select("SELECT MASTER_ACC.ACC_NAME,SALES_QTN_MONTH_VIEW.* FROM SQTN_HEAD LEFT JOIN SQTN_BODY ON SQTN_HEAD.SQTNHID = SQTN_BODY.SQTNHID  LEFT JOIN SALES_QTN_MONTH_VIEW ON SALES_QTN_MONTH_VIEW.ACC_CODE = SQTN_HEAD.ACC_CODE LEFT JOIN MASTER_ACC ON SALES_QTN_MONTH_VIEW.ACC_CODE = MASTER_ACC.ACC_CODE WHERE 1=1 $strWhere GROUP BY SALES_QTN_MONTH_VIEW.ACC_CODE");*/
               ...
               if($this->code=='series code'){
                  ...
                  }else if($this->code=='month' || $this->code=='item code month' || $this->code=='acc code month' || $this->code=='acc item code month' || $this->code=='item acc code month'){
