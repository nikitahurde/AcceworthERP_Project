...
class PurchaseMonthlyGrnExport implements FromCollection, WithHeadings,ShouldAutoSize,WithCustomStartCell,WithEvents
{
    /**
    ...
    */
    ...
    public function headings():array{
      ...
      //DB::enableQueryLog();
                  ...
                  $CNTR_BODY = DB::select("SELECT GRN_HEAD.VRDATE,GRN_HEAD.PFCT_CODE,GRN_HEAD.PLANT_CODE,GRN_HEAD.TRAN_CODE,MASTER_ACC.ACC_NAME AS ACC_NAME,GRN_BODY.* FROM GRN_HEAD LEFT JOIN GRN_BODY ON GRN_HEAD.GRNHID = GRN_BODY.GRNHID LEFT JOIN MASTER_ACC ON GRN_HEAD.ACC_CODE = MASTER_ACC.ACC_CODE WHERE 1=1 $strWhere");
            ...
            /*
               ...
               if($this->code=='series code'){
                  //  DB::enableQueryLog();
                    ...
                    $data = DB::select("SELECT GRN_MONTH_VIEW.* FROM GRN_HEAD LEFT JOIN GRN_BODY ON GRN_HEAD.GRNHID = GRN_BODY.GRNHID LEFT JOIN    GRN_MONTH_VIEW ON    GRN_MONTH_VIEW.ACC_CODE = GRN_HEAD.ACC_CODE LEFT JOIN MASTER_ACC ON MASTER_ACC.ACC_CODE = GRN_HEAD.ACC_CODE WHERE 1=1 $strWhere GROUP BY GRN_HEAD.ACC_CODE");
                  }else if($this->code=='tax code'){
