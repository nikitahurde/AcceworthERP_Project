...
$("#equip_code").bind('change', function () {
  var Equipcode =  $(this).val();

  if(Equipcode==''){
  
     $('#equip_code').css('border-color','#d2d6de');
     $('#job_type').css('border-color','#d2d6de');
     $('#equip_code').css('border-color','#ff0000').focus();
     //$('#asset_code').css('border-color','#d2d6de');
     }else{
      $('#equip_code').css('border-color','#d2d6de');
      $('#job_type').css('border-color','#ff0000').focus();
     // $('#asset_code').css('border-color','#ff0000').focus();
     }


  var xyz = $('#EquipList option').filter(function() {

    return this.value == Equipcode;
  ...
  if(msg=='No Match'){
     $(this).val('');
     $('#equip_name').val('');
     //document.getElementById("eqpcode_err").innerHTML = 'The Equipment code field is required.';
      $('#getAccCode').val('');
      $('#emplList').empty();
      $('#getEquipCode').val('');
      $('#getEquipName').val('');
    ...
    $('#equip_name').val(msg);
    // document.getElementById("eqpcode_err").innerHTML = '';
     $('#getEquipCode').val(Equipcode);
     $('#getEquipName').val(msg);
...
$("#Plant_code").bind('change', function () {
  ...
  if(msg=='No Match'){
     ...
     $('#appndplantbtn').append('<button type="button"  data-toggle="modal" data-target="#plant_detail" onclick="billty.getplantdata(Plantdetailsurl)" class="btn btn-xs btn-info gly-radius" data-original-title="" title="" style="padding: 0px 5px 0px 5px;"> <i class="fa fa-info" aria-hidden="true" style="font-size: 13px;"></i></button>');
  ...
  $("#storage_code").bind('change', function () {
  var Storagecode =  $(this).val();
  //console.log(Seriescode);
  var xyz = $('#storageList option').filter(function() {

    return this.value == Storagecode;
  ...
  if(msg=='No Match'){
     $(this).val('');
     $('#storage_name').val('');
      document.getElementById("storage_code_errr").innerHTML = 'The Storage code field is required.';
      $('#Plant_code').prop('readonly',true);
    ...
    $('#storage_name').val(msg);
     document.getElementById("storage_code_errr").innerHTML = '';
