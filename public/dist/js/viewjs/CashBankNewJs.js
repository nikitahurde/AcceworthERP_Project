  ...
  $('#vr_type').on('change',function(){
    ...
    if( $("#vr_type option:selected").val()!=''){
      ...
      if(vrType == 'Payment'){
        ...
        $('#submitReceiptpdf').addClass('submitBtnLog');
      }else if(vrType == 'Receipt'){
        $('.discription').val('By -');
        $('#vrTypeData').val(vrType);
        $('#IsChequeBookOpen').val('NO');
        $("#chequeNoList1").empty();
        $('#submitdatapdf').addClass('submitBtnLog');
        $('#submitReceiptpdf').removeClass('submitBtnLog');
  ...
  function glcodeNameData(slno){
    ...
    var vrType= $('#vr_type').val();
      ...
      if(msg=='No Match'){
        ...
        $('#simulation_btn,#submitdata,#submitdatapdf,#submitReceiptpdf').prop('disabled',true);
...
/* -------------- account code ------------------ */
  ...
  function GetAccountCode(Accid){
    ...
    if(vrType == 'Payment'){
      ...
      $('#cr_amount'+Accid).prop('readonly',true);
      $('#payemntAdvice'+Accid).removeClass('payAdcBtn');
    }else if(vrType == 'Receipt'){
      $('#dr_amount'+Accid).prop('readonly',true);
      $('#cr_amount'+Accid).prop('readonly',false);
      $('#payemntAdvice'+Accid).addClass('payAdcBtn');
    }
            ...
            success:function(data){
              ...
              if (data1.response == 'error') {
                ...
                if(data1.data==''){
                  ...
                  console.log('data1.data[0].GL_CODE',data1.data[0].GL_CODE);
                  if(glCodechk == ''){
                  ...
                  if(data1.data_gl == ''){
                    ...
                    //$('#glCodeName'+Accid+',#genrl_name'+Accid+'').val('');
                  ...
                  if(data1.data[0].BILLTRACKAPPLY == 1){
                    $('#billTkDr'+Accid).html('<button type="button" class="btn btn-primary btn-xs viewBilTrak" id="ViewBillTrack'+Accid+'" data-toggle="modal" data-target="#ViewBT_Detail'+Accid+'" onclick="detailBillTrack('+Accid+')">Bill Track </button><div id="AplyIconBT'+Accid+'" style="padding-top: 5px;">');
                  }else{
                ...
                if(data1.data_glList == ''){
                  
                }else{
                  $('#glCodeName'+Accid+',#genrl_name'+Accid+'').val('');
                  $('#glCodeNameList'+Accid).empty();

                  $.each(data1.data_glList, function(k, getData){
                      $("#glCodeNameList"+Accid).append($('<option>',{
                        value:getData.GL_CODE,
                        'data-xyz':getData.GL_NAME,
                        text:getData.GL_NAME+' ['+getData.GL_NAME+']'
    ...
    if((costCTag == 'YES') && (acctTag == 'YES')){
      ...
      var accCode  = $('#acc_code'+srNo).val();
        ...
        if(costCode && accCode){
          ...
          $('#submitdata,#submitdatapdf,#submitReceiptpdf,#simulation_btn').prop('disabled',false);
          ...
          $('#submitdata,#submitdatapdf,#submitReceiptpdf,#simulation_btn').prop('disabled',true);
  ...
  function GetDebitAmount(debitid){
        ...
        $(".dr_amount").each(function () {
          ...
          if (!isNaN(this.value) && this.value.length != 0) {
            ...
            $('#submitdata,#submitdatapdf,#submitReceiptpdf,#deletehidn,#addmorhidn,#simulation_btn').prop('disabled',false);
          }else{
              /*$('#cr_amount'+debitid).prop('readonly',false);*/
              $('#tds_rate'+debitid).prop('disabled',true);
              $('#submitdata,#submitdatapdf,#submitReceiptpdf,#deletehidn,#addmorhidn,#simulation_btn').prop('disabled',true);
          }   
...
/* ---------------- show cheque date ------------- */
  ...
  function changedate(datevalue){
      ...
      var intType =  $('#inst_type'+datevalue).val();

      var xyz = $('#InstTypeList'+datevalue+' option').filter(function() {
      ...
      var msg = xyz ?  xyz : 'No Match';
      if(msg == 'No Match'){
        $('#intTypeName'+datevalue).val('');
      }else{
        $('#intTypeName'+datevalue).val(msg);
      }
      ...
      if(insttype=='CH'){
        $('#cheque_no'+datevalue).prop('readonly',false);
        $("#showdate"+datevalue).removeClass('datehide');
        $('#chquedate'+datevalue).val(vrDate);
        ...
        if(vr_type == 'Payment'){
          ...
          var chqNoListUrl = folderName+'/get-vr-sequence-by-series';
          $.ajax({
