...
$('#ref_no').on('input',function(){
        ...
        if(ref_no){
          ...
          $('#ref_date').css('border-color','#ff0000');
           ...
           $('#ref_no').css('border-color','#ff0000');
        }
...
$('#valid_to_dt').on('change',function(){
        ...
        if(valid_to_dt){
          ...
          $('#valid_to_dt').css('border-color','#d2d6de');
          $('#route_code1').css('border-color','#ff0000').focus();

        }else{
           $("#getvalidtoDate").val('');
           $('#route_code1').css('border-color','#d2d6de');
           $('#valid_to_dt').css('border-color','#ff0000').focus();
        }
...
$("#account_code").bind('change', function () {
  ...
  if(msg=='No Match'){
     ...
     document.getElementById("acccode_err").innerHTML = 'The Custmer code field is required.';
      $('#getAccCode').val('');
      $('#emplList').empty();
      $('#freight_order_no').val('');
      $('#empName').html('');
      $('#emplyeeName').val('');
      $('#freight_order_no').prop('readonly',true);
      $('#vehicle_type').css('border-color','#d2d6de');
    ...
    $('#account_name').val(msg);
     ...
     $('#getAccName').val(msg);
     $('#freight_order_no').prop('readonly',false);
     $('#getVrNo').val(vrseqnum);
     $('#getTransCode').val(transcode);
...
$("#vehicle_type").on('change', function () {
  var val =  $(this).val();
  var xyz = $('#vehciletypeList option').filter(function() {
  ...
  if(msg=='No Match'){
     ...
     $('#getVehicleType').val('');
      document.getElementById("vehicletype_err").innerHTML = 'The Freight order field is required.';
     
       $('#ref_no').css('border-color','#d2d6de');
      $('#vehicle_type').css('border-color','#ff0000').focus();
  }else{
    
     $('#getVehicleType').val(val);
    document.getElementById("vehicletype_err").innerHTML = '';
     $('#vehicle_type').css('border-color','#d2d6de');
     $('#vehicle_type').css('border-color','#d2d6de');
